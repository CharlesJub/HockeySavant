...
def players_data():
    ...
    position_query = (
        ...
        if position != "Goalie" and len(POSITIONS[position]) == 1
            ...
            f'players."primaryPositionCode" IN {POSITIONS[position]}'
            if position != "Goalie"
            else None
        )
    )
    ...
    for season_info in seasons:
        if str(season_info["season"]) == draft_year:
            ...
            if games_played < season_info["gamesPlayed"]:
                ...
                main_team = season_info["teamName"]["default"]
...
def skater_percentile(player_id):
    ...
    percentile_stats = [
        dict(zip(percentile_column_names, row)) for row in cursor.fetchall()
    ]
